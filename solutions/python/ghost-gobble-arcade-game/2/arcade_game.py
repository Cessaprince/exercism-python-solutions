...
def win(has_eaten_all_dots, power_pellet_active, touching_ghost):
    ...
    #instatiate the 'lost_game' variable
    lost_game = False

    #change the 'lost_game' variable to True that is if the conditions met for loss is met.
    if (power_pellet_active is not True and touching_ghost is True):

        #'lost_game' becomes True
        lost_game = True

    #set the condition for winning for if the pac_man has eaten all dots
    if has_eaten_all_dots is True and not lost_game:
        return True
